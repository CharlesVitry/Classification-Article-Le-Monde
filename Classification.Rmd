...
Algorithmes : CART, RandomForest, 3ème.
...
Avec ni gestion des manquants, ni gestion des fautes d'orthographes à réaliser.
...
```{r}
cp_expand = expand.grid( .cp = seq(from = 0, to = 0.01, by = 0.00005)) 
```
...
library(ROCR)
library(pROC)
pRF_CV <- predict(modele_rf_cv, newdata=base_modelisation[-nb_lignes,], type= "prob")
pRF_CV <- pRF_CV[,1]
```
Matrice de confusion
```{r}
...
auc(base_modelisation[-nb_lignes,]$category, pRF_CV)
```
Un Auc de 0.88 a été trouver avec notre jeu de test, c'est moins bien que notre deuxième modèle.
Il doit exister un moyen d'optimiser cela.


```{r}
#Visualisation de la prédiction
plot(pRF_CV ~ category, data=base_modelisation[-nb_lignes,], xlab="Observe",
       ylab="Predis")
```
