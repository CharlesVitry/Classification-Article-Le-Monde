...
Cette unique variable explicative est inexploitable, 
...
Premièrement, visualisons graphiquement si nos variables sont très corrélés avec une heatmap : 
```{r}
source("http://www.sthda.com/upload/rquery_cormat.r")
 
rquery.cormat(base_modelisation[, c(2:440)],graphType="heatmap")
```
Pour complèter cela, on réalise une analyse en composante principale
```{r}
...
Nous pouvons affirmer que les données sont très dispersés.
...
Notre critère de répartition est le GINI.
...
```{r}
Besttree <-rpart(category~. , data = base_modelisation[nb_lignes,], cp=8e-04, minsplit = 10)
...
```{r}
p <- predict(Besttree, newdata=base_modelisation[-nb_lignes,], type= "class")
length(p)
...
p1 <- predict(Besttree, newdata=base_modelisation[-nb_lignes,], type= "prob")
p1 <- p1[,1]
...
```{r}
modele_rf$importance[order(modele_rf$importance[,1], decreasing = TRUE), ]
```
