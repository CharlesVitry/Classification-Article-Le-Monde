...
Les paramètres que nous optimiserons seront le mtry et le ntree, le maxnodes (nbre de feuilles) aurait pu aussi être optimiser.
...
#plot(modele_rf)
```
On observe une stagnation de l'erreur à partir de __ arbres, nous utiliserons cette donnée pour la cross validation(on ne la calcule pas directement sur la cross validation par manque de puissance de calcul)
...
modele_mtry
mtry_expand <- expand.grid(.mtry = modele_mtry)
...
```
<br> </br>
On re-execute avec la valeur correct du mtry.
```{r}
require(caret)
require(doSNOW)
...
si nous avions plus de puissance de calcul nous aurions aussi optimiser gamma.
```{r}
...
```
Un Auc de 0.85 a été trouver avec notre jeu de test.
...
Et 0.95 avec le jeu d'entrainement.
...
```
Un Auc de 0.92 a été trouver avec notre jeu de test, et un AUC de 0.96 avec notre jeu d'entrainement.
...
```
AUC de 0.84 pour un SVM avec une fonction kernel en linéaire sur notre jeu de test, un AUC de 0.95 sur notre jeu d'entrainement.
...
```
Auc de 0.81 et 0.92 avec le jeu d'entrainement.
